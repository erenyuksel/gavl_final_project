...
const EditEvent = () => {
  ...
  const eventEvaluationCriteria = useSelector(
    (state) => state.rubric.evaluationCriteria,
  )
  const { id } = useParams()
  ...
  useEffect(() => {
    ...
    const getEventData = async () => {
      try {
        ...
        const response_rubrics = await JudgeAxios.get(
          `/rubrics/${response.data.rubrics}`,
        )
        const response_eval_by_rubric = await JudgeAxios.get(
          `/rubrics/${response.data.rubrics}/evaluations/`,
        )
        // console.log("fetch EVAL rubrics....................", response_eval_by_rubric.data)
        if (
          response_eval_by_rubric.data &&
          response_eval_by_rubric.data.length > 0
        ) {
    ...
    if (!errorFlag) {
      try {
        ...
        if (updatedPanelists) {
          ...
          const patchResponse = await JudgeAxios.patch(`/events/${id}/`, {
            judges: panelistIDs,
          })
          dispatch(setEvaluationCriteria([]))
          dispatch(setJudges([]))
  ...
  try {
    ...
    // Remove all projects from the event
    await JudgeAxios.patch(`events/${id}/`, { projects: [] })
    
      setSuccessMessage('All projects have been deleted successfully!');
      setTimeout(() => {
        setSuccessMessage('');
      }, 3000); // 3 seconds
    } catch (error) {
        ...
        <div className="flex flex-col gap-5 w-full lg:w-2/3 xl:w-1/2 p-5">
          ...
          {rubrics && (
            <div className="flex flex-col items-center">
              ...
              <p>Update the evaluation criterias of your event.</p>
            </div>
          )}
          ...
          {rubrics &&
            rubrics.map((obj) => (
              <div key={obj.uuid} className="flex flex-col">
                <EditEventRubric
                  rubric={obj}
                  removeRubric={() => handleRemoveRubric(obj.uuid)}
                  isDisabled={isRubricsDisabled}
                />
              </div>
            ))}

          <EditEventRubric
            rubric={rubricPropsInformation}
            addRubric={handleAddRubric}
            isDisabled={isRubricsDisabled}
          />
          ...
          <div className="w-full p-4 flex flex-row justify-center gap-6 border">
            <button
              className="btn bg-red-500 mt-8"
